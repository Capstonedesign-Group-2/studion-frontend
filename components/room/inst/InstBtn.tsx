...
const InstBtn = ({ type, selectedInst, imgPath, setSelectedInst }: Props) => {
  const selectInst = useCallback(() => {
    if (selectedInst === type) {
      setSelectedInst('')
    } else {
      setSelectedInst(type)
    }
  }, [selectedInst, type, setSelectedInst])

  return (
    <div className="flex flex-col relative rounded-lg items-center group hover:cursor-pointer"
      onClick={selectInst}
    >
      <div className="bg-studion-400 overflow-hidden aspect-video relative rounded-lg w-full shadow-lg">
