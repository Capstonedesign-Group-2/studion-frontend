...
import { getPostList, getUserPostList } from "../../redux/actions/post";
const EditCard = ({pevPost, userId}) => {
    ...
    const [post, setPost] = useState({
        ...
        image: pevPost.images[0] ? pevPost.images[0] : null,
        audio: pevPost.audios[0] ? pevPost.audios[0] : null,
        id: pevPost.id,
    })
    const onToggle = () => {
    ...
    const onChange = (e) => {
        if (e.target.files.length) {
            ...
            setPost({
                ...post,
                image: pevPost.images,
                audio: pevPost.audios
            });
        }
    }
    ...
    const onPosting = () => {
        ...
        http.post(`/posts/${post.id}`,formData, config)
        .then(res => {
            if(userId !== undefined)
    ...
    useEffect(() => {
        ...
        if (((post.image && !toggle && !post.audio ) || (post.audio && post.image && toggle))) {
            let imgEl = document.querySelector('.img_box')
            if(post.image.link){
                imgEl.src = post.image.link
                return ;
            } else {
            ...
            <div className="mx-auto lg:flex-row h-full flex flex-col">
                ...
                <div className="flex relative flex-col items-center h-full justify-center w-full max-screen-w-lg mx-auto lg:mx-0 lg:max-w-3xl ">
                    <div className="w-full flex justify-center">
                        <label htmlFor="inputFiles" className="cursor-pointer" style={ (post.image || post.audio) ? {display: "none"} : {display:"block"}}>
                    ...
                    {/* 토글 */}
                {
                    ...
                    { ((post.image && !toggle && !post.audio ) || (post.audio && post.image && toggle))
                        &&
                        <div className="">
