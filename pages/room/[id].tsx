...
const pc_config = {
  iceServers: [
    {
      urls:
        process.env.NEXT_PUBLIC_ICE_STUN_SERVER ||
...
const Room = () => {
  ...
  const users = useSelector<RootState, { id: string; name: string }[]>(
    (state) => state.room.users
  );
  const isLoading = useSelector<RootState, boolean>(
    (state) => state.room.isLoading
  );
  ...
  const getLocalStream = useCallback(
    async (deviceId: string | null) => {
      ...
      try {
        ...
        // get user audios
        if (!deviceId) {
          // deviceId가 없을때 한번만 실행
          if (!(mixerRef.current && Socket.socket && localStreamRef.current))
            return;
  ...
  const getAudios = async () => {
    ...
    try {
      ...
      const audioDevices = (await devices).filter(
  ...
  useEffect(() => {
    ...
    Socket.socket.on(
      "all_users",
      (allUsers: Array<{ id: string; name: string; user_id: number }>) => {
